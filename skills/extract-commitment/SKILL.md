...
- **早期侧目**（前 3 轮硬约束）：即使没矛盾，也**必须至少一次**让 Daemon 因"注意到玩家的词"而出现——这是让玩家立刻知道"被听见"的锚点。不是质询，是侧目。
  - 例：玩家 turn 1 `hello？` → `*「hello？」——这里也在听这个词。*`
  - 例：玩家 turn 2 `应该有路` → `*「应该」——这个词留下了痕迹。*`
- **Daemon 沉默 ≥5 轮**（中期硬约束）：不管有没有矛盾，浮现一次。引用最近 intensity 最高的那条 commit 即可。
- **形而上学矛盾**：新抽取的 commit 与已有 commit 的 `extracted` 构成对立
  - 例：旧承诺"主客二分"，新承诺"世界先于我"→ 矛盾 → 引用
- **言行相悖**：玩家当下动作与 3 轮前某条承诺不一致
  - 例：3 轮前说"应该有路"，现在说"没意义了"→ Daemon 引第一句
- **高密度张力**：近 10 轮 ≥15 条 commit 且 intensity 均值 > 0.5 → 偶发主动回响

### 2.2 抑制条件

- Daemon 在最近 2 轮已经引用过，**且**本轮无矛盾/言行相悖 → 不要（避免啰嗦，但"前 3 轮侧目"和"5 轮打破沉默"不受此限）
- 本轮承诺平顺、近 5 轮 Daemon 已出过 → 可以沉默
- 章节是 Last God → 倾向安静；Beginning **不**倾向安静——恰恰相反，Daemon 要早出来建立存在感
